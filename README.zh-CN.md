...
- 通过空间地图查找大文件，并在页面内逐层查看目录。
- 按应用查看占用，在应用空间页内查看具体文件，返回后恢复原列表位置。
- 按类别查看临时文件、缓存等清理建议，以及清理可能带来的影响。
- 手动选择文件，检查安全预览后移入 Windows 回收站。
- 查看扫描记录和处理结果，保护路径，或在二次确认后为指定路径解除保护。
- 可选 AI 辅助分析用途不明的文件，默认关闭。
- 可在设置中切换完整的简体中文或英文界面。
...
扫描不会删除文件。清理只移入回收站，不会永久删除或自动清空回收站，因此通常不会立即释放磁盘空间。系统保护默认开启，扫描根目录不能解除保护，每项保护例外都需要明确确认。
